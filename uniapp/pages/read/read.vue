...
import {getItem,setItem,removeItem,setFullscreen} from '@/common/appcomact.js';
export default {
	components: {
		battery,
		virtualList
	...
	onLoad(obj) {
		...
		this.bookName = this.bookInfo.name;
		this.getSystemInfo();
		this.initPage();
		this.updateviews();
	},
	...
	methods: {
		...
		async initPage() {
			...
			console.log("end getThreeChapter");
		 ...
		 **/
		calcHeight() {
			console.log("calcHeight----contentHeight",this.contentHeight);
			if (this.contentHeight) {
				let lineHeight = this.fontSize * this.lineHeight;
				console.log("存在this.contentHeight--lineHeight="+lineHeight);
				...
				console.log("存在，inherHeight="+this.innerHeight);
			} else {
				console.log("不存在，异步计算");
					...
					this.$nextTick(() => {
						...
						query
							...
							.boundingClientRect(data => {
								...
								console.log("原始lineHeight="+this.lineHeight);
		...
		*/
		async getDirectoryList(showLoading) {
			...
			await this.$u.api
				...
				.then(res => {
					...
					if (!res.data || !res.data.zjlist || res.data.zjlist.length < 1) {
						return this.$u.toast('该章节没有内容');
					}
		...
		getOneChapter(chapterId, showLoading) {
			...
			return new Promise((resolve, reject) => {
				...
				if (hastext) {
					...
					this.tmpChapter.text = hastext; //模拟数据
					...
					this.$u.api
						...
						.then(res => {
							console.log("从服务器获取章节数据成功，chapterId="+chapterId);
							if (res.status !== 0) {
								console.log(this.bookId, chapterId);
								uni.showToast({
									title: res.msg,
									mask: true
								});
							...
							if (!res.data || !res.data.content) {
								...
								this.showtips = '该章节不存在正文';
								return this.$u.toLocaleString('该章节不存在正文');
							}
							// console.log(res.data);

							this.tmpChapter.text = res.data.content; //模拟数据
							if (this.simplified === 2) {
								//切换为繁体  注意：这里默认数据库中是简体字
								this.tmpChapter.text = traditionalized(this.tmpChapter.text);
							}
		...
		async getThreeChapter(index) {
			...
			console.log("this.curChapter",this.curChapter);
